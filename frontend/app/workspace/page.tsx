...
const views: Array<{ id: View; label: string; short: string }> = [
  ...
  { id: "standards", label: "Standards", short: "ST" },
...
export default function WorkspacePage() {
  ...
  function signOut() {
    clearToken();
    clerkSignOut(() => router.push("/login"));
  }
  ...
  const loadWorkspace = useCallback(async () => {
    setError("");
    try {
      const [productRows, reviewRows, batchRows, jobRows, analyticsReport, referenceRows, evalRows] = await Promise.all([
        ...
        apiFetch<EvaluationRun[]>("/evaluations?limit=1"),
      ]);
      ...
      setError(message);
      if (!isSignedIn && message.toLowerCase().includes("authentication")) router.replace("/login");
    } finally {
      setLoading(false);
    }
  }, [router, selectedProductId, isSignedIn]);

  useEffect(() => {
    if (!isLoaded) return;
    void loadWorkspace();
  }, [loadWorkspace, isLoaded]);
    ...
    <main className="workspace-shell">
      <aside className="workspace-sidebar">
        ...
        <div className="workspace-account">
          <span>{user?.role || (isSignedIn ? "Reviewer" : "LOCAL MODE")}</span>
          <strong>{user?.full_name || (isSignedIn ? "Clerk User" : "Development workspace")}</strong>
          {isSignedIn ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <UserButton afterSignOutUrl="/login" />
              <button onClick={signOut} type="button">Sign out</button>
            </div>
          ) : <Link href="/login">Configure sign in</Link>}
        </div>
      ...
      <section className="workspace-main">
        ...
        {loading ? <div className="workspace-loading">Loading catalog operations...</div> : (
          <div className="workspace-content">
            ...
            {view === "standards" && <StandardsView datasets={datasets} evaluations={evaluations} onRefresh={loadWorkspace} onNotice={setNotice} onError={setError} />}
...
function ProductsView({ products, product, selectedId, setSelectedId, onCreated, onRefresh, onNotice, onError }: {
  ...
  async function deleteProduct() {
    ...
    setBusy(true);
    try {
      await apiFetch(`/products/${selectedId}`, { method: "DELETE" });
      setSelectedId("");
      await onRefresh();
          ...
          <>
            <div className="record-detail-head">
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button className="solid-command" disabled={busy} onClick={deleteProduct} type="button" style={{ background: '#e53e3e' }}>Delete</button>
...
function StandardsView({ datasets, evaluations, onRefresh, onNotice, onError }: any) {
  ...
  return (
    <div style={{ padding: '24px', overflowY: 'auto', height: '100%' }}>
      <div className="workspace-band" style={{ marginTop: 0 }}>
        <div className="band-heading">
          <div>
            <span>Customer content system</span>
            <h2>Standards & Evaluation</h2>
          </div>
          <div className="status-chip ready" style={{ minHeight: '32px', fontSize: '10px' }}>
            {datasets.length}/{referenceTypes.length} standards loaded
          </div>
        </div>
        <div style={{ padding: '16px 18px', color: 'var(--ink)', fontSize: '12px' }}>
          <p style={{ margin: 0 }}>Control the approved vocabulary, delivery structure, and measurable output quality.</p>
        </div>
      </div>
      ...
      <section className="workspace-band">
        <div className="band-heading">
          ...
          <form style={{ display: 'flex', gap: '10px', alignItems: 'center' }} onSubmit={uploadReference}>
            <select style={{ height: '38px', padding: '0 9px', fontSize: '10px', border: '1px solid var(--line)', background: '#fff' }} value={datasetType} onChange={(event) => setDatasetType(event.target.value as typeof datasetType)}>
            ...
            />
            <button className="solid-command" type="submit" disabled={!referenceFile || busy === "reference"}>
        ...
        <div>
          {referenceTypes.map((item) => {
            ...
            const deliveryColumns = item.id === "ground_truth"
              ...
              : 0;
            return (
              <div className="field-row" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }} key={item.id}>
                <div>
                  <strong style={{ display: 'block', fontSize: '12px', marginBottom: '4px' }}>{item.label}</strong>
                  <small style={{ color: 'var(--muted)', fontSize: '10px' }}>{item.note}</small>
                </div>
                <div>
                  {dataset ? (
                    <div style={{ textAlign: 'right' }}>
                      <strong style={{ display: 'block', color: 'var(--ink)', fontSize: '12px' }}>{dataset.row_count.toLocaleString()} rows</strong>
                      <span style={{ fontSize: '9px', color: 'var(--muted)' }}>{deliveryColumns ? `${deliveryColumns} delivery columns` : dataset.filename}</span>
                    </div>
                  ) : <span className="status-chip">Not loaded</span>}
                </div>
              </div>
            );
          })}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px", marginTop: "24px", paddingBottom: "24px" }}>
        <section className="workspace-band" style={{ marginTop: 0 }}>
          <div className="band-heading">
            <div>
          ...
          <div style={{ padding: '16px 18px' }}>
            <p style={{ color: 'var(--muted)', fontSize: '11px', margin: '0 0 16px 0' }}>Every visible worksheet is read, and each product keeps its workbook, sheet, and row lineage.</p>
            <form style={{ display: "flex", gap: "10px", alignItems: 'center' }} onSubmit={importCatalog}>
              <input
                type="file"
              ...
              />
              <button className="solid-command" type="submit" disabled={!catalogFile || busy === "catalog"}>
        ...
        <section className="workspace-band" style={{ marginTop: 0 }}>
          <div className="band-heading">
            ...
            <button className="solid-command" disabled={!groundTruth || busy === "evaluation"} onClick={runEvaluation} type="button">
              {busy === "evaluation" ? "Evaluating..." : "Run evaluation"}
            </button>
          </div>
          <div style={{ padding: '16px 18px' }}>
            {!groundTruth && <p style={{ color: 'var(--muted)', fontSize: '11px', margin: 0 }}>Load the Input vs Delivery Format workbook to enable scoring.</p>}
            {latest && (
              <>
                <div className="record-scores" style={{ borderBottom: 'none', display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: "16px", paddingTop: "16px", borderTop: "1px solid var(--line)" }}>
                  <span style={{ fontSize: '10px', color: 'var(--ink)' }}>{latest.matched_items} of {latest.total_items} products matched</span>
                  <button style={{ background: "none", border: "none", color: "var(--orange-dark)", cursor: "pointer", textDecoration: "underline", fontSize: '10px', padding: 0 }} onClick={() => apiDownload(`/evaluations/${latest.id}/report.csv`, `ferrox-evaluation-${latest.id}.csv`)}>
                    Download row report
                  </button>
                </div>
              </>
...
function Score({ label, value }: { label: string; value: number }) {
  return (
    <div className="metric" style={{ minHeight: 'auto', padding: '12px 8px', background: '#f4f3ee', border: '1px solid var(--line)', borderRadius: '4px', flex: 1, textAlign: 'center' }}>
      <strong style={{ margin: 0, fontSize: '16px', display: 'block', color: 'var(--ink)' }}>{Math.round(value * 100)}%</strong>
      <span style={{ fontSize: '8px', color: 'var(--muted)', textTransform: 'uppercase', fontFamily: '"Courier New", monospace', fontWeight: 700, display: 'block', marginTop: '6px' }}>{label}</span>
    </div>
  );
}